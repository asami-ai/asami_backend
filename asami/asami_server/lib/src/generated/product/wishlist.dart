...
class WishlistUpdateTable extends _i1.UpdateTable<WishlistTable> {
  WishlistUpdateTable(super.table);
...
class WishlistTable extends _i1.Table<_i1.UuidValue> {
  WishlistTable({super.tableRelation}) : super(tableName: 'wishlists') {
    updateTable = WishlistUpdateTable(this);
  ...
  late final WishlistUpdateTable updateTable;
...
class WishlistRepository {
  ...
  /// Updates a single [Wishlist] by its [id] with the specified [columnValues].
  /// Returns the updated row or null if no row with the given id exists.
  Future<Wishlist?> updateById(
    _i1.Session session,
    _i1.UuidValue id, {
    required _i1.ColumnValueListBuilder<WishlistUpdateTable> columnValues,
    _i1.Transaction? transaction,
  }) async {
    return session.db.updateById<Wishlist>(
      id,
      columnValues: columnValues(Wishlist.t.updateTable),
      transaction: transaction,
    );
  }

  /// Updates all [Wishlist]s matching the [where] expression with the specified [columnValues].
  /// Returns the list of updated rows.
  Future<List<Wishlist>> updateWhere(
    _i1.Session session, {
    required _i1.ColumnValueListBuilder<WishlistUpdateTable> columnValues,
    ...
    return session.db.updateWhere<Wishlist>(
      columnValues: columnValues(Wishlist.t.updateTable),
